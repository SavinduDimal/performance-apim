...
#
# ----------------------------------------------------------------------------
# Run API Manager Performance Tests (Docker Version)
# ----------------------------------------------------------------------------
...
function before_execute_test_scenario() {
    ...
    echo "Starting APIM service using Docker"
    ssh $apim_ssh_host "./apim/apim-docker-start.sh -m $heap"
}

function after_execute_test_scenario() {
    write_server_metrics apim $apim_ssh_host org.wso2.carbon.bootstrap.Bootstrap
    # For Docker version, logs are in the mounted volume
